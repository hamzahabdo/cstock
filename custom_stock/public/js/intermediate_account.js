...
frappe.ui.form.on("Stock Entry", {
  ...
  before_save(frm) {
    ...
    if (!frm.doc.from_warehouse && !cur_frm.doc.target_warehouse) {
      if (frm.doc.purpose == "Material Transfer" && frm.doc.add_to_transit) {
        ...
        frm.set_value("source_warehouse", frm.doc.items[0].s_warehouse);
      }
    }
  },
  purpose(frm) {
  ...
  get_inventory_items(frm, qty_type) {
    let qtys,
      _qtys,
      warehouse,
      btn_name = "Get Items From Stock Inventory";
  ...
  to_warehouse(frm) {
    if (frm.doc.purpose == __("Material Receipt")) {
      frm
        .set_value("target_warehouse", frm.doc.to_warehouse)
        .then(frm.events.set_warehouse_acronym(frm, frm.doc.to_warehouse, "sw"));
  ...
  prepare_RAW(frm) {
    ...
    reset_fields(frm, "", "from_warehouse", "to_warehouse", "source_warehouse", "target_warehouse");
  },
  prepare_MI(frm) {
    toggle_fields(frm, false, "source_warehouse", "target_warehouse", "to_warehouse");
    toggle_fields(frm, true, "from_warehouse");
    reset_fields(frm, "", "from_warehouse", "to_warehouse", "source_warehouse", "target_warehouse", "branch");
  },
  prepare_MR(frm) {
        ...
        if (r.message) {
          ...
          frappe.run_serially([
            ...
            () => {
              ...
              frm.set_value("to_warehouse", _entry.target_warehouse).then(() => {
