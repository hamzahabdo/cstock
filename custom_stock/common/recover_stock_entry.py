...
def recover_stock_entry(between_branches = False,to_date= '',deploy_changes = False):
    stock_entry_transactions = get_stock_entry_transactions(between_branches,to_date)
    if not between_branches:
        current_accoutn_in_gle = check_current_account_in_gle(stock_entry_transactions,deploy_changes)
        return current_accoutn_in_gle
    else:
        current_account_between_branches_in_gle = check_current_account_between_branches_in_gle(stock_entry_transactions,deploy_changes)
...
def get_stock_entry_transactions(between_branches = False,to_date = ''):
    all_transactions  = frappe.get_list("Stock Entry",fields=['name','purpose','source_warehouse','target_warehouse','add_to_transit','outgoing_stock_entry'],filters={'purpose':'Material Transfer','posting_date':['<=',to_date],'docstatus':1})
    ...
    for transaction in all_transactions: 
        if(transaction.source_warehouse and transaction.target_warehouse):
            ...
            source_current_warehouse = get_current_account(transaction.source_warehouse)
            target_current_warehouse = get_current_account(transaction.target_warehouse)
            
            # if ((transaction.add_to_transit and (not between_branches)) or transaction.name == 'MT-QHM-QHW-0222-55' or transaction.name == 'MT-QHM-QHW-0222-64' or (transaction.outgoing_stock_entry)):
...
def fix_gl_entry_for_stock_entry(voucher_no_for_se,deploy_changes):
    if(deploy_changes):
        make_reverse_gl_entries(voucher_type='Stock Entry',voucher_no=voucher_no_for_se)
        se_doc = frappe.get_doc('Stock Entry',voucher_no_for_se)
        # # frappe.call({'doc':se_doc, 'method':"make_gl_entries"})
        frappe.call(getattr(se_doc, "make_gl_entries"), **frappe.local.form_dict)
...
def check_current_account_in_gle(transactions,deploy_changes):
    ...
    for transaction in transactions:
        gl_entry_list = frappe.get_all('GL Entry',fields=['name','account','voucher_no','posting_date'],filters={'is_cancelled':0,'voucher_no':transaction.name})
        ...
        for gl_entry in gl_entry_list:
            if(gl_entry.account == transaction.current_warehouse):
                ...
                fix_gl_entry_for_stock_entry(gl_entry.voucher_no,deploy_changes)
...
def check_current_account_between_branches_in_gle(transactions,deploy_changes):
    ...
    for transaction in transactions:
        ...
        if not exist_current_warehouse:
            ...
            # make_reverse_gl_entries(voucher_type='Stock Entry',voucher_no=transaction.name)
            # se_doc = frappe.get_doc('Stock Entry',transaction.name)
            # frappe.call(getattr(se_doc, "make_gl_entries"), **frappe.local.form_dict)

            # for exist_gl_entry in exist_gl_entries:
                # frappe.delete_doc('GL Entry', exist_gl_entry.name)
                # pass
                # make_reverse_gl_entries(voucher_type:'Stock Entry',voucher_no:cur_frm.doc.name)
                # print(exist_gl_entries)
