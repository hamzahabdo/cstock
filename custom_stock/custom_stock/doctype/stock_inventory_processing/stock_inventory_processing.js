...
frappe.ui.form.on("Stock Inventory Processing", {
  ...
  get_inventory_items(frm) {
    ...
    if (frm.doc.warehouse) {
      ...
      let docs_list = easy_pos.utils.get_child_table_elements(frm, "stock_inventory_documents", "document_name"),
        docs_string;

      if (docs_list.length != 0) {
        docs_string = easy_pos.utils.array_to_db_string(docs_list);
        ...
        if (frm.doc.all_items) {
          ...
          let groups_list = easy_pos.utils.get_child_table_elements(frm, "item_groups", "item_group"),
            groups_string;
          if (groups_list.length != 0) {
            groups_string = easy_pos.utils.array_to_db_string(groups_list);
...
easy_pos.utils.check_child_table_duplicates(
  "Stock Inventory Document",
...
easy_pos.utils.check_child_table_duplicates(
  "Stock Inventory Processing Item Group",
  "item_groups",
  "item_group",
  "groups_found",
  "groups_removed_doc"
