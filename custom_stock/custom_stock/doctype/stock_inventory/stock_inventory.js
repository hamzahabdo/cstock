...
frappe.ui.form.on('Stock Inventory', {
	...
	set_autosave_interval(frm) {
		if (frm.doc.title && frm.doc.warehouse && frm.doc.items
			&& frm.doc.items[0].item_code != undefined && frm.doc.interval > 0) {
	...
	set_autosave_counts(frm) {
		if (frm.doc.title && frm.doc.warehouse && frm.doc.items
			&& frm.doc.items[0].item_code != undefined && frm.doc.items_count > 0) {
			frm.doc.qty_count++;
	...
	scan_barcode_or_item_code(frm) {
		...
		function show_description(item_code, idx, exist = null) {
			if (exist) {
				scan_barcode_field.set_new_description(__(`Row #${idx}: Item <b>${item_code}</b> Qty increased by ${(frm.doc.punch_count || 1)}`));
			} else {
...
{ dt: 'Stock Inventory Last Item', dn: 'last_items' }].forEach(doctype => {
	nct.utils.add_missing_values_based_on_trigger(doctype.dt, 'item_code', (frm, d) => {
